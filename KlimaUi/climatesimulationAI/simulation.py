...
from climatesimulationAI.Training.Training import Training
from climatesimulationAI.Training.PreProcessing import PreprocessingTrainData
...
def postprocessing(pred_data, years, i, sealevel=False, withcmip6=False, new=False, awi=False, newsea=True):
    ...
    if not withcmip6:
        if i == 115200 or i == 115201:
            ...
            startvalue = np.load(
                '../KlimaUi/climatesimulationAI/Training/PreProcessing/developingnotebooks/train_data_1880_2014.npy',
                mmap_mode='r+')[
                             i, 83] - 273.15
    else:
        if awi:
        ...
        if i == u or i == (u + 1):
            ...
            if awi:
                startvalue = np.load(
                    '../KlimaUi/climatesimulationAI/Training/PreProcessing/developingnotebooks/train_data_2014_2100_awi.npy',
                    mmap_mode='r+')[
                                 i, 83] - 273.15  # train_data_2014_2100_ssp2
            else:
                startvalue = np.load(
                    '../KlimaUi/climatesimulationAI/Training/PreProcessing/developingnotebooks/train_data_2014_2100_ssp2.npy',
...
def calc_withrainforestemissions(future_inputs, rainforestemission_co2, temperature_pred, X, start, years, gafsize,
    ...
    for i in range(temperature_pred[0, :].size - 1):
        temperature_denormtemp[:, i] = denorm(temperature_pred[:, i],
                                              np.load('train_data_2014_2100_ssp2.npy', mmap_mode='r+')[i, 0] - 273.15,
                                              sealevel=False,
                                              nccmip6=True,
                                              fred93=True)
...
def pred(ghgchanges, start, end, numberofghgs=6,
         ...
         partly_anaerobe=False, partanaeorbe=None, over200=False, withpermafrost=True):
    ...
    # over200: whether to use a different model due to the range of emissions, boolean (default false)
