...
def my_norm(a, kind,maxtrainreal,mintrainreal):
    ...
    # maxtrainreal: maxima from training data for different kinds, np.array
    # mintrainreal: minima from training data for different kinds, np.array
...
def postprocessing(pred_data, years, i, sealevel=False, withcmip6=False, new=False, awi=False, newsea=True, startvalue1=None, startvaluenonawi=None):
    ...
    # newsea: if the new sea level model is used, boolean (optional)
    # startvalue:startvalue for grid points imported from training data, float
    # startvalueawi: startvalue for grid points imported from training data (AWI Grid), float
    # startvaluenonawi: startvalue for grid points imported from training data (non-AWI Grid), float
...
def preprocessing(future_inputa, years, gafsize, numberoffeatures, actfactora, s=None,maxtrainreal=None,mintrainreal=None):
        ...
        if s is not None:
            framesall5[i] = PreprocessingTrainData.generateframes(5,
                                                                  PreprocessingTrainData.activationforgaf(
                                                                      my_norm(future_inputa[i], s, maxtrainreal, mintrainreal), actfactora))
        else:
            framesall5[i] = PreprocessingTrainData.generateframes(5,
                                                                  PreprocessingTrainData.activationforgaf(
                                                                      my_norm(future_inputa[i], i,maxtrainreal, mintrainreal), actfactora))
...
def calc_withrainforestemissions(future_inputs, rainforestemission_co2, temperature_pred, X, start, years, gafsize,
                                 numberoffeatures, actfactor, p_rainforest, maxtrainreal, mintrainreal):
    ...
    traindatassp2 = np.load('KlimaUi/climatesimulationAI/Training/PreProcessing/trainingdata/train_data_2014_2100_ssp2.npy',mmap_mode='r+')[:, 0]

    temperature_denormtemp = np.ones(temperature_pred.shape)
    for i in range(temperature_pred[0, :].size - 1):
        temperature_denormtemp[:, i] = denorm(temperature_pred[:, i],traindatassp2[i] - 273.15,
                                              sealevel=False,
                                              nccmip6=True,
                                              fred93=True)
    ...
    for i in range(1, X[:, 0, 0, 0].size):
        ...
        if temp > tippingpointrainforest and rainforesttriggerd == False:
            ...
            Xnew = preprocessing(withrainforest_future_inputs, years, gafsize, numberoffeatures, actfactor, None, maxtrainreal, mintrainreal)
...
def calc_withpermafrostemissions(future_inputs, p, permafrostemission_co2, temperature_pred, X, start, years, gafsize,
                                 numberoffeatures, actfactor, anerobe=False, permafrostemission_ch4=None,maxtrainreal=None, mintrainreal=None):
    # future_inputs: emissions in all used ghgs as calculated before, np.array, shape (numberoffeatures, years.size)
    ...
    for i in range(1, X[:, 0, 0, 0].size):
        ...
        if temp > tippingpointpermafrost and permafrosttriggerd == False:
            ...
            Xnew = preprocessing(withpermafrost_future_inputs, years, gafsize, numberoffeatures, actfactor, None, maxtrainreal, mintrainreal)
...
def transformgeocoordinates(lat, lon,lats,lons):
...
def pred(ghgchanges, start=2014, end=2114, numberofghgs=6,
    ...
    if not exceededmodelrange:
        ...
        X = preprocessing(future_inputs, years, gafsize, numberoffeatures, actfactor,None,maxtrainreal, mintrainreal)
        ...
        if not onlyindata:
            ...
            if withtippingpoints == True and permafrosttriggerd == False:
                ...
                if withpermafrost:
                    ...
                    for i in range(len(p_geo_lat)):
                        ...
                        for j in range(len(p_geo_lat[i])):
                            p_temp[j] = transformgeocoordinates(p_geo_lat[i][j], p_geo_lon[i][j],lats, lons)
                        p[i] = np.array(p_temp)
                ...
                if rainforestused:
                    ...
                    rainforest = calc_withrainforestemissions(future_inputs, rainforestemission_co2, temperature_pred,
                                                              ...
                                                              p_rainforest, maxtrainreal, mintrainreal)
                    if rainforest is not None:
                        ...
                        # calculate new combined GAF for model input
                        Xnew = preprocessing(future_inputs, years, gafsize, numberoffeatures, actfactor, None, maxtrainreal, mintrainreal)
                ...
                if withpermafrost:
                        ...
                        if not permafrosttriggerd_parzelle[i]:
                            ...
                            if anaerobe:
                                permafrost_parzelle = calc_withpermafrostemissions(future_inputs, p[i],
                                                                                   ...
                                                                                   permafrostemission_ch4[i],maxtrainreal=maxtrainreal,mintrainreal=mintrainreal)
                            else:
                                permafrost_parzelle = calc_withpermafrostemissions(future_inputs, p[i],
                                                                                   permafrostemission_co2[i],
                                                                                   temperature_pred, X, start, years,
                                                                                   gafsize, numberoffeatures, actfactor,maxtrainreal,mintrainreal)
                        else:
                    ...
                    try:
                        ...
                        Xnew = preprocessing(future_inputs, years, gafsize, numberoffeatures,
                                             actfactor, None, maxtrainreal, mintrainreal)
            ...
            cmip6postproc = True
            startvalue1 = np.load('KlimaUi/climatesimulationAI/Training/PreProcessing/trainingdata/train_data_1880_2014.npy',mmap_mode='r+')[:, 83] - 273.15
            startvaluenonawi = np.load('KlimaUI/climatesimulationAI/Training/PreProcessing/trainingdata/train_data_2014_2100_ssp2.npy',mmap_mode='r+')[:, 83] - 273.15

            for i in range(temperature_pred[0, :].size):
                if i < temperature_pred[0, :].size - 1:
                    temperature_denorm[:, i] = postprocessing(temperature_pred[:, i], years, i, withcmip6=cmip6postproc,new=new, awi=awi,startvalue1= startvalue1[i], startvaluenonawi=startvaluenonawi[i])
                else:
            ...
            if with_oldmodel:
                ...
                for i in range(temperature_pred_old[0, :].size):
                    if i < temperature_pred_old[0, :].size - 1:
                        temperature_denorm_old[:, i] = postprocessing(temperature_pred_old[:, i], years, i,
                                                                      withcmip6=cmip6postproc, new=new, awi=awi,startvalue1= startvalue1[i],startvaluenonawi=startvaluenonawi[i])
                    else:
                        temperature_denorm_old[:, i] = postprocessing(temperature_pred_old[:, i], years, i,
            ...
            if predsea:
                ...
                all5temperature = preprocessing(np.array(newtemp[:, u - 1:u].transpose()), years, gafsizesea,
                                                1, 0.8, s=6, maxtrainreal=maxtrainreal, mintrainreal=mintrainreal)
                ...
                for i in range(newseas[0, :].size):
                    sealevel_denorm[:, i] = postprocessing(newseas[:, i], years, i, sealevel=True,
                                                           withcmip6=withcmip6, awi=awi, newsea=newsea,startvaluenonawi=startvaluenonawi[i])
